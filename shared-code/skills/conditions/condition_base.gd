...
const CONDITION_PATH: String  = "res://skills/conditions"
const OK: String = ""
...
static func load(condition_name: String) -> Condition:
    ...
    if not script:
        printerr("Unable to load condition script!",
            {"path":script_path})

    var condition: Condition = script.new() as Condition

    print("Loaded conditon.", {"condition":condition.desc})

    return condition


func check(_caster:Entity, _target:Entity, _context:Dictionary) -> bool:
    print("Tried to call check on condition base! Verify that check function is impelmented on the condition scripts. Condition will return true.")
