...
var area_scene: Resource = preload("res://shared-code/skills/aoe.tscn")
...
func _init(config_file) -> void:
    ...
    var err = config.load("res://skills/configs/%s.cfg" % file.to_lower())

    if err:
        printerr("Failed to load skill!", {"skill":config_file,"error":err})
...
func cast() -> MessageCastResult:
    ...
    if skill_type == "targeted":
        ...
        print("Invalid cast type:", {"type": self.skill_type, "skill": file})
...
func _handle_area(result: MessageCastResult) -> MessageCastResult:
        print("Casted area skill.", {"skill":file})
